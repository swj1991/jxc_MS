...
public class SalesSelect extends JInternalFrame {
	private JTextField txtSeltSalesName;
	...
	public SalesSelect() {
		...
		txtSeltSalesName = new JTextField();
		txtSeltSalesName.setColumns(10);
		
		table = new JTable();
		table.setEnabled(false);
		table.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
		
		String[] tableHeads = new String[]{"销售编号", "销售日期", "顾客姓名", "负责人", "总金额"};
		...
		btnSeltCusSelect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String selectName = txtSeltSalesName.getText();
				try {
					if(selectName.trim().length() == 0) {
						JOptionPane.showMessageDialog(null, "请输入需要查询的顾客姓名!");
						return;
					}
					List<List> all = DAOFactory.getISalesOrderDAOInstance().findByCustomer(selectName);
					
					int size = all.size();
					if(size == 0) {
						JOptionPane.showMessageDialog(null, "没有找到顾客名字包含  '" + selectName + "' 的销售信息!");
						return;
					}
		...
		JButton btnSeltCusSelectAll = new JButton("显示全部销售信息");
		btnSeltCusSelectAll.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				txtSeltSalesName.setText("");
				try {
					int num = tableModel.getRowCount();
					for (int i = 0; i < num; i++)
						tableModel.removeRow(0);
					List<List> all = DAOFactory.getISalesOrderDAOInstance().findByCustomer("");
					int size = all.size();
					JOptionPane.showMessageDialog(null, "找到 " + size + " 条信息");
					updateTable(all,tableModel);
				} catch (Exception e1) {
				...
				.addGroup(groupLayout.createSequentialGroup()
					...
					.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
						...
						.addGroup(groupLayout.createSequentialGroup()
							...
							.addComponent(txtSeltSalesName, GroupLayout.PREFERRED_SIZE, 114, GroupLayout.PREFERRED_SIZE)
							.addPreferredGap(ComponentPlacement.RELATED)
			...
			groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup()
					...
					.addGroup(groupLayout.createParallelGroup(Alignment.BASELINE)
						.addComponent(lblSeltCusName)
						.addComponent(txtSeltSalesName, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
						.addComponent(btnSeltCusSelect)
						.addComponent(btnSeltCusSelectAll))
					.addPreferredGap(ComponentPlacement.RELATED)
